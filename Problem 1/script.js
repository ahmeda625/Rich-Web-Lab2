...
function validateInput(name, mobile, email) //function that validates the name, email and mobile number
{
    if (name.length === 0) 
    {
    ...
    {
        error.innerHTML = "Name should be less than 20 characters."
        return false
    } 
    else 
    if (!name.match(letter)) 
    {
        error.innerHTML = "Name should have only letters"
        return false
    } 
    else 
    if (mobile.length !== 10) 
...
addButton.addEventListener("click", function() 
{
    ...
    {
        if (validateInput(nameInput.value, mobileInput.value, emailInput.value)) 
        {
            insertValid(nameInput.value, mobileInput.value, emailInput.value)
        }
    }
})
